...
//        mealsTo = filteredByStreams(meals, LocalTime.of(0, 0), LocalTime.of(23, 59),2000);
    ...
    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Long> amountCalloriesPerDay = meals.stream()
                .filter(meal -> TimeUtil.isBetweenHalfOpen(meal.getDateTime().toLocalTime(), startTime, endTime))
                .collect(
                        groupingBy(meal -> meal.getDateTime().toLocalDate(), summingLong(UserMeal::getCalories))
                );

        return meals.stream().map(meal -> new UserMealWithExcess(
                meal.getDateTime(),
                meal.getDescription(),
                meal.getCalories(),
                amountCalloriesPerDay.get(meal.getDateTime().toLocalDate()) > caloriesPerDay
        )).collect(toList());
    }
}
